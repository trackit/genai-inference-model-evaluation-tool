...
describe('DatasetUploadUseCase', () => {
  ...
  let mockUploadDataset: ReturnType<typeof vi.fn>;

  beforeEach(() => {
    reset();

    mockUploadDataset = vi.fn().mockResolvedValue({
      dataset_id: 'test-id',
      sample_count: 10,
      has_summary: false,
      has_class: false,
    ...
    register(tokenDatasetService, {
      useValue: { uploadDataset: mockUploadDataset },
    });

    useCase = new DatasetUploadUseCaseImpl();
  });
