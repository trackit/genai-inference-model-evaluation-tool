...
const TEST_MODEL = 'anthropic.claude-3-5-sonnet-20241022-v2:0';

const validLaunchRequest = (): EvaluationRequest => ({
  dataset_id: 'test-dataset-id',
  models: [{ identifier: TEST_MODEL }],
...
describe('Weight Configuration', () => {
  ...
  describe('Weight normalization', () => {
    ...
    it('should normalize partial weights and use defaults for missing values', async () => {
      ...
      expect(normalizedWeights.accuracy).toBeCloseTo(0.8, 4);
      expect(normalizedWeights.latency).toBeCloseTo(0, 4);
      expect(normalizedWeights.cost).toBeCloseTo(0.2, 4);
    });
  ...
  describe('Edge cases', () => {
    ...
    it('should accept a mix of default and custom models', async () => {
      ...
      expect(
        bedrockModelValidationService.resolveModelsForPersistence,
      ).toHaveBeenCalledWith(request.models);
...
const setup = () => {
  ...
  const bedrockModelValidationService = inject(
    tokenFakeBedrockModelValidationService,
  );
  vi.spyOn(bedrockModelValidationService, 'resolveModelsForPersistence');
  return {
